...
<script>
  ...
  import {getMeetSignUp,updateMeetSignUp} from '../../api/service';
  ...
  export default {
        ...
        this.$refs.addForm.validate((valid) => {
          if (valid) {
            this.$confirm('确认提交吗？', '提示', {}).then(() => {
              ...
              console.log(para);
              updateMeetSignUp(para).then((res) => {
                this.$notify({
                  title: '成功',
                  message: '提交成功',
                  duration:2500,
    ...
    mounted() {
       let meetSignUpId=this.$route.query.meetSignUpId;
       let para = {meetSignUpId: meetSignUpId};
       getMeetSignUp(para).then((data) => {
