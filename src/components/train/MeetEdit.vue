...
<script>
  ...
  import {updateMeet,getMeet} from '../../api/service';
  ...
  export default {
    ...
    mounted() {
      this.addForm.meetId=this.$route.query.meetId;
       let para = {meetId: this.addForm.meetId};
       getMeet(para).then((data) => {
        this.addForm=data.retData;
        this.titleDate.bussinessId=this.addForm.meetPicId;
        fileinfos({"bussinessId":this.addForm.meetPicId}).then((data) => {
